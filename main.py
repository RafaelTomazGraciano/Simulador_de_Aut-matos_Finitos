...
def delta(q, a, transicoes): #Função delta que executa as transicoes do automato
    ...
    for transicao in  transicoes:
        if int(transicao['from']) == q and transicao['read'] == a or transicao['read'] == None:
            resultados.append(int(transicao['to']))
    return resultados
...
deterministico = e_deterministico(dadosJson)

#definindo os estados
estadoInicial = dadosJson['initial']
estadoFinal = [(final_state) for final_state in dadosJson['final']]
